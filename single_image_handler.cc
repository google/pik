...
float SingleImageManager::BlockSaliency(size_t row, size_t col) const {
  // TODO(user): Determine if the indexing below is correct.
...
void SingleImageManager::GetAcStrategy(
    float butteraugli_target, const ImageF* quant_field, const Image3F* src,
    const Image3F* coeffs_init, ThreadPool* pool, AcStrategyImage* ac_strategy,
    PikInfo* aux_out) {
  if (num_passes_ == 0) {
    FindBestAcStrategy(butteraugli_target, quant_field, src, coeffs_init, pool,
