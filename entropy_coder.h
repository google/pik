...
const int kNaturalCoeffOrder[kBlockSize + 16] = {
...
static const int kNumContexts = 128 + kOrderContexts * (32 + 120);
...
std::string EncodeCoeffOrders(const int* PIK_RESTRICT order,
                              PikInfo* PIK_RESTRICT pik_info);

std::string EncodeImage(const Image3S& img, int stride, PikImageSizeInfo* info);

struct Token {
  Token(int c, int s, int nb, int b)
...
std::vector<Token> TokenizeCoefficients(const int* orders,
                                        const ImageI& quant_ac,
                                        const Image3S& coeffs,
                                        const Image3B& block_ctx);

std::string BuildAndEncodeHistograms(
    int num_contexts, const std::vector<std::vector<Token> >& tokens,
