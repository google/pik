...
void RGBPixelToYUVOpsin(double r, double g, double b, int bits,
  ...
  const double rm = (kOpsinAbsorbanceMatrix[0] * rl +
                     kOpsinAbsorbanceMatrix[1] * gl +
                     kOpsinAbsorbanceMatrix[2] * bl);
  const double gm = (kOpsinAbsorbanceMatrix[3] * rl +
                     kOpsinAbsorbanceMatrix[4] * gl +
                     kOpsinAbsorbanceMatrix[5] * bl);
  const double bm = (kOpsinAbsorbanceMatrix[6] * rl +
                     kOpsinAbsorbanceMatrix[7] * gl +
...
// Output range: [0 .. (1<<bits)-1]
template <typename T>
void RGBPixelToYUVOpsin(T r, T g, T b, int bits,
                        uint16_t *y, uint16_t *u, uint16_t *v) {
...
void YUVOpsinImageToRGB(const Image3U& yuv, int bit_depth, Image3<T>* rgb) {
  for (int y = 0; y < yuv.ysize(); ++y) {
    ...
    for (int x = 0; x < yuv.xsize(); ++x) {
      YUVOpsinPixelToRGB(row_yuv[0][x], row_yuv[1][x], row_yuv[2][x], bit_depth,
...
void RGBImageToYUVOpsin(const Image3<T>& rgb, int bit_depth, Image3U* yuv) {
  for (int y = 0; y < rgb.ysize(); ++y) {
    ...
    for (int x = 0; x < rgb.xsize(); ++x) {
      RGBPixelToYUVOpsin(row_rgb[0][x], row_rgb[1][x], row_rgb[2][x], bit_depth,
...
Image3U YUVOpsinImageFromRGBLinear(const Image3F& rgb, int out_bit_depth) {
  ...
  for (int y = 0; y < yuv.ysize(); ++y) {
    ...
    for (int x = 0; x < yuv.xsize(); ++x) {
      ...
      RGBPixelToYUVOpsin(rd, gd, bd, out_bit_depth,
                         &row_yuv[0][x], &row_yuv[1][x], &row_yuv[2][x]);
    }
  }
  return yuv;
