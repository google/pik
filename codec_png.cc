 ...
 private:
  ...
  static Status DecodeBase16(const char* key, const char* encoded,
    ...
    *type = key + strlen(kKey);
    ...
    // Header: type and number of bytes
...
class ColorEncodingReaderPNG {
 public:
  ...
  Status operator()(const PaddedBytes& bytes, const bool is_gray,
                    const ColorManagement& cms, Metadata* metadata,
                    ColorEncoding* c_original) {
    ...
    if (have_srgb_) {
      ...
      return cms.SetProfileFromFields(c_original);
    }
    ...
    if (cms.SetFromParams(params_, c_original)) return true;
 ...
 private:
  ...
  Status DecodeEXIF(const unsigned char* payload, const size_t payload_size,
    ...
    if (!metadata->exif.empty()) {
      fprintf(
          stderr,
          "WARNING: overwriting EXIF (%zu bytes) with eXIf (%zu bytes)",
          metadata->exif.size(), payload_size);
    }
...
Status CheckGray(const LodePNGColorType type, bool* is_gray) {
  switch (type) {
    case LCT_GREY:
    case LCT_GREY_ALPHA:
      *is_gray = true;
...
Status CheckAlpha(const LodePNGColorType type, bool* has_alpha) {
  switch (type) {
    case LCT_GREY:
    case LCT_RGB:
      *has_alpha = false;
...
Status DecodeImagePNG(const PaddedBytes& bytes, CodecInOut* io) {
  unsigned w, h;
  PNGState state;
  ...
  PIK_RETURN_IF_ERROR(CheckGray(color_mode.colortype, &is_gray));
  PIK_RETURN_IF_ERROR(CheckAlpha(color_mode.colortype, &has_alpha));
  ...
  PIK_RETURN_IF_ERROR(reader(bytes, is_gray, io->Context()->cms, &io->metadata,
                             &io->dec_c_original));
...
Status EncodeImagePNG(const CodecInOut* io, const ColorEncoding& c_desired,
  ...
  const ExternalImage external(
      io->Context(), io->color(), io->c_current(), c_desired, io->HasAlpha(),
      alpha, io->enc_bits_per_sample, big_endian, temp_intervals);
