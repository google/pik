...
// only use PIK_TARGET_Portable, in which case Supported() just returns that.
#if PIK_ARCH_X64
...
TargetBits InstructionSets::Supported() {
  ...
  if (PIK_LIKELY(supported)) {
    return supported;
  }

  supported = PIK_TARGET_Portable;
  ...
  if ((flags & kGroupAVX2) == kGroupAVX2) {
    supported |= PIK_TARGET_AVX2;
  }
  if ((flags & kGroupSSE41) == kGroupSSE41) {
    supported |= PIK_TARGET_SSE41;
  }
#endif  // PIK_ARCH_X64

  // We will never store a 0, because PIK_TARGET_Portable is always set.
