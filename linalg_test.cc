...
Image<T> RandomMatrix(const size_t xsize, const size_t ysize, Random& rng,
                      const T vmin, const T vmax) {
  Image<T> A(xsize, ysize);
...
template <typename T, typename Random>
Image<T> RandomSymmetricMatrix(const size_t N, Random& rng, const T vmin,
                               const T vmax) {
  Image<T> A = RandomMatrix<T>(N, N, rng, vmin, vmax);
